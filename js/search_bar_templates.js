				...
				<div id="textual${canvasID}_container" colspan="12" title="Scene Description">
					...
					<div id="div_textual${canvasID}">
						...
						<div  class="Icon-inside">
							<input id="textual${canvasID}" type="text" class="textualquery${canvasID} font-normal" style="width: 100%; padding: 8px 30px 8px 30px; border-radius: 6px; border: 1px solid #ccc;" placeholder="${txtSearchTitle}" onkeydown="if(event.keyCode==13){event.preventDefault(); var textQuery=this.value.trim(); if(textQuery.length>0){if(document.getElementById('isTranslate${canvasID}').checked){translateText(textQuery, ${canvasID});}else{queryByTextual();}} this.blur(); return false;}">
